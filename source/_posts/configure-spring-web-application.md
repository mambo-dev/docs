...
웹 애플리케이션이 요구하는 기능에 따라 스프링 프레임워크에서 제공하는 기능을 설정해야할 것이 많습니다. 간단하게나마 몇가지 부분에 대해서 알아보았는데도 참 많다고 느껴집니다. 이외에도 다음과 같은 것들은 웹 애플리케이션에서 필요한 기능일 수 있습니다.

- 애플리케이션 내부적으로 주기적으로 동작해야하는 스케줄링
- 이메일 발송
- 다른 웹 애플리케이션으로 요청하기 위한 HTTP 요청 클라이언트
- 데이터베이스로 접근하기 위한 데이터 엑세스
- 분산 웹 애플리케이션을 위한 세션 관리
- 웹 애플리케이션 보안 적용

다음 글에서는 [스프링 애플리케이션에 고급 기능](../advanced-feature-spring-application)을 적용해보도록 하겠습니다.
